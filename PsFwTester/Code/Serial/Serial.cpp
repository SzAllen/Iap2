...
// is generated when using the _RPTF and MY_ASSERTE macros.
...
CSerial::~CSerial ()
{
	...
	{
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::~CSerial - Serial port not closed\n");
...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	if (m_hFile)
	{
		...
		MY_RPTF(_CRT_WARN,"CSerial::Open - Port already opened\n");
		return m_lLastError;
	}
	...
	if (m_hFile == INVALID_HANDLE_VALUE)
	{
		...
		MY_RPTF(_CRT_WARN, "CSerial::Open - Unable to open port\n");
		return m_lLastError;
	}

#ifndef SERIAL_NO_OVERLAPPED
	// We cannot have an event handle yet
	MY_ASSERTE(m_hevtOverlapped == 0);
	...
	if (fOverlapped)
	{
		...
		if (m_hevtOverlapped == 0)
		{
			...
			m_lLastError = ::GetLastError();
			MY_RPTF(_CRT_WARN,"CSerial::Open - Unable to create event\n");
	...
	if (dwInQueue || dwOutQueue)
	{
		...
		// 16 bytes large.
		MY_ASSERTE(dwInQueue >= 16);
		MY_ASSERTE(dwOutQueue >= 16);

		if (!::SetupComm(m_hFile,dwInQueue,dwOutQueue))
		{
			// Display a warning
			long lLastError = ::GetLastError();
			MY_RPTF(_CRT_WARN,"CSerial::Open - Unable to setup the COM-port\n");
	...
	if (::GetDefaultCommConfig(lpszDevice,&commConfig,&dwSize))
	{
		...
		{
			// Display a warning
			MY_RPTF(_CRT_WARN,"CSerial::Open - Unable to set default communication configuration.\n");
		}
	}
	else
	{
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::Open - Unable to obtain default communication configuration.\n");
	}
...
LONG CSerial::Close (void)
{
	...
	{
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::Close - Method called when device is not open\n");
		return m_lLastError;
	}
...
LONG CSerial::Setup (EBaudrate eBaudrate, EDataBits eDataBits, EParity eParity, EStopBits eStopBits)
{
	...
	if (m_hFile == 0)
	{
		...
		MY_RPTF(_CRT_WARN,"CSerial::Setup - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::Setup - Unable to obtain DCB information\n");
		return m_lLastError;
	}
	...
	if (!::SetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::Setup - Unable to set DCB information\n");
		return m_lLastError;
	}
...
LONG CSerial::SetEventChar (BYTE bEventChar, bool fAdjustMask)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::SetEventChar - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetEventChar - Unable to obtain DCB information\n");
		return m_lLastError;
	}
	...
	if (!::SetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetEventChar - Unable to set DCB information\n");
		return m_lLastError;
	}
...
LONG CSerial::SetMask (DWORD dwEventMask)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::SetMask - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::SetCommMask(m_hFile,dwEventMask))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetMask - Unable to set event mask\n");
		return m_lLastError;
	}
...
LONG CSerial::WaitEvent (LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::WaitEvent - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!m_hevtOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::WaitEvent - Overlapped I/O is disabled, specified parameters are illegal.\n");
		return m_lLastError;
	}
	...
	MY_ASSERTE(!m_hevtOverlapped || HasOverlappedIoCompleted(lpOverlapped));
	...
	if (!::WaitCommEvent(m_hFile,LPDWORD(&m_eEvent),lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
			MY_RPTF(_CRT_WARN,"CSerial::WaitEvent - Unable to wait for COM event\n");
			return m_lLastError;
		}
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			default:
				...
				// Issue an error and quit
				MY_RPTF(_CRT_WARN,"CSerial::WaitEvent - Unable to wait until COM event has arrived\n");
				return m_lLastError;
			}
		}
	}
...
LONG CSerial::SetupHandshaking (EHandshake eHandshake)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::SetupHandshaking - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetupHandshaking - Unable to obtain DCB information\n");
		return m_lLastError;
	}
	...
	if (!::SetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetupHandshaking - Unable to set DCB information\n");
		return m_lLastError;
	}
...
LONG CSerial::SetupReadTimeouts (EReadTimeout eReadTimeout)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::SetupReadTimeouts - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::GetCommTimeouts(m_hFile,&cto))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetupReadTimeouts - Unable to obtain timeout information\n");
		return m_lLastError;
	}
	...
	if (!::SetCommTimeouts(m_hFile,&cto))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::SetupReadTimeouts - Unable to set timeout information\n");
		return m_lLastError;
	}
...
CSerial::EBaudrate CSerial::GetBaudrate (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetBaudrate - Device is not opened\n");
		return EBaudUnknown;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetBaudrate - Unable to obtain DCB information\n");
		return EBaudUnknown;
	}
...
CSerial::EDataBits CSerial::GetDataBits (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetDataBits - Device is not opened\n");
		return EDataUnknown;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetDataBits - Unable to obtain DCB information\n");
		return EDataUnknown;
	}
...
CSerial::EParity CSerial::GetParity (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetParity - Device is not opened\n");
		return EParUnknown;
	}
	return m_EParity;
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetParity - Unable to obtain DCB information\n");
		return EParUnknown;
	}
...
CSerial::EStopBits CSerial::GetStopBits (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetStopBits - Device is not opened\n");
		return EStopUnknown;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetStopBits - Unable to obtain DCB information\n");
		return EStopUnknown;
	}
...
DWORD CSerial::GetEventMask (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetEventMask - Device is not opened\n");
		return 0;
	}
...
BYTE CSerial::GetEventChar (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetEventChar - Device is not opened\n");
		return 0;
	}
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetEventChar - Unable to obtain DCB information\n");
		return 0;
	}
...
CSerial::EHandshake CSerial::GetHandshaking (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetHandshaking - Device is not opened\n");
	...
	if (!::GetCommState(m_hFile,&dcb))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetHandshaking - Unable to obtain DCB information\n");
		return EHandshakeUnknown;
	}
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	MY_ASSERTE(!lpOverlapped || pdwWritten);
	...
	*pdwWritten = 0;
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Write - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!m_hevtOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Write - Overlapped I/O is disabled, specified parameters are illegal.\n");
		return m_lLastError;
	}
	...
	if (!::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
			MY_RPTF(_CRT_WARN,"CSerial::Write - Unable to write the data\n");
			return m_lLastError;
		}
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwWritten,FALSE))
				{
					...
					MY_RPTF(_CRT_WARN,"CSerial::Write - Overlapped completed without result\n");
					return m_lLastError;
				}
				break;
			...
			default:
				...
				// Issue an error and quit
				MY_RPTF(_CRT_WARN,"CSerial::Write - Unable to wait until data has been sent\n");
				return m_lLastError;
			}
		}
	}
...
LONG CSerial::Read (void* pData, size_t iLen, DWORD* pdwRead, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	MY_ASSERTE(!lpOverlapped || pdwRead);
	...
	*pdwRead = 0;
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Read - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!m_hevtOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Read - Overlapped I/O is disabled, specified parameters are illegal.\n");
		return m_lLastError;
	}
	...
	if (!::ReadFile(m_hFile,pData,iLen,pdwRead,lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
			MY_RPTF(_CRT_WARN,"CSerial::Read - Unable to read the data\n");
			return m_lLastError;
		}
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwRead,FALSE))
				{
					...
					MY_RPTF(_CRT_WARN,"CSerial::Read - Overlapped completed without result\n");
					return m_lLastError;
				}
				break;
			...
			default:
				...
				// Issue an error and quit
				MY_RPTF(_CRT_WARN,"CSerial::Read - Unable to wait until data has been read\n");
				return m_lLastError;
			}
		}
	}
...
LONG CSerial::Purge()
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Purge - Device is not opened\n");
		return m_lLastError;
	}
	...
	if (!::PurgeComm(m_hFile, PURGE_TXCLEAR | PURGE_RXCLEAR))
	{
		// Set the internal error code
		m_lLastError = ::GetLastError();
		MY_RPTF(_CRT_WARN,"CSerial::Purge - Overlapped completed without result\n");
	}
...
LONG CSerial::Break (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::Break - Device is not opened\n");
		return m_lLastError;
	}
...
CSerial::EError CSerial::GetError (void)
{
	...
	if (m_hFile == 0)
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetError - Device is not opened\n");
	...
	if (!::ClearCommError(m_hFile,&dwErrors,0))
	{
		...
		// Issue an error and quit
		MY_RPTF(_CRT_WARN,"CSerial::GetError - Unable to obtain COM status\n");
		return EErrorUnknown;
	}
...
bool CSerial::GetCTS (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetCTS - Unable to obtain the modem status\n");
		return false;
	}
...
bool CSerial::GetDSR (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetDSR - Unable to obtain the modem status\n");
		return false;
	}
...
bool CSerial::GetRing (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetRing - Unable to obtain the modem status");
		return false;
	}
...
bool CSerial::GetRLSD (void)
{
	...
	if (!::GetCommModemStatus(m_hFile,&dwModemStat))
	{
		...
		// Display a warning
		MY_RPTF(_CRT_WARN,"CSerial::GetRLSD - Unable to obtain the modem status");
		return false;
	}
...
void CSerial::FormatComPort(CString& comPort)
{
	LPCTSTR lpComPor = comPort.LockBuffer();
	comPort = FormatComPort(lpComPor);
}

CString CSerial::FormatComPort(LPCTSTR lpComPort)
{
	CString strRet;
	int comNum = _wtoi(lpComPort + 3);

	if(wcslen(lpComPort) == 5)
	{
		if(comNum >= 10)
		{
			strRet.Format(L"\\\\.\\COM%d", comNum);
		}
		else
		{
			strRet.Format(L"COM%d", comNum);
...
int CSerial::GetSystemPorts(CStringArray& portList, CStringArray* pNameList)
{
	portList.RemoveAll();

    CRegKey RegKey;   
    //打开注册表
	if(ERROR_SUCCESS != RegKey.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\serenum\\Enum", KEY_READ))
	{
		return 0;
	}
    ...
    if(ERROR_SUCCESS != RegKey.QueryDWORDValue(_T("Count"), dwCount))
    {
        return 0;//检索失败，退出
    }
    ...
    for(DWORD i = 0;i < dwCount; i++)
	{
		...
		//COM端口数检索成功，进一步检索在设备管理器里出现的文字
		memset(wcValue, 0, sizeof(0));
		size = _MAX_PATH;
		if(ERROR_SUCCESS != RegKey.QueryStringValue(strReg.LockBuffer(), wcValue, &size))
		{
			break;
		}
		//TRACE(L"%s\n", wcValue);

		strReg.Format(L"SYSTEM\\CurrentControlSet\\Enum\\%s", wcValue);
		if(ERROR_SUCCESS != enumComKey.Open(HKEY_LOCAL_MACHINE, strReg, KEY_READ))
		{
			return 0;
		}

		//检索COM口具体说明文字，即FriendlyName的值
		size = _MAX_PATH;
		if(ERROR_SUCCESS != enumComKey.QueryStringValue(_T("FriendlyName"), wcValue, &size))
		{
			break;
		}
		//TRACE(L"%s\n", wcValue);
		if(pNameList)
			pNameList->Add(CString(wcValue));

		//检索COM口号码
		WCHAR* lpCom = NULL;
		lpCom = wcsstr(wcValue, L"(COM");
		if(NULL == lpCom) continue;
		lpCom++;	//Remove the head char '('      
		lpCom[wcslen(lpCom) - 1] = L'\0';	//Remove the tail char ')'    
