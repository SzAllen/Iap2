...
CSerialEx::~CSerialEx()
{
	...
	{
		// Display a warning
		MY_RPTF(_CRT_WARN, "CSerialEx::~CSerialEx - Serial port not closed\n");
...
LONG CSerialEx::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fStartListener)
{
	...
	if (m_hevtOverlappedWorkerThread == 0)
	{
		...
		MY_RPTF(_CRT_WARN,"CSerialEx::Open - Unable to create event\n");
...
LONG CSerialEx::StartListener (void)
{
	...
	if (m_hThread == 0)
	{
		...
		{
			// Display a warning
			MY_RPTF(_CRT_WARN, "CSerialEx::StartListener - Unable to start COMM watcher thread\n");
...
DWORD CSerialEx::ThreadProc (void)
{
	...
	do
	{
		...
		if (::WaitForSingleObject(m_hevtOverlappedWorkerThread,INFINITE) != WAIT_OBJECT_0)
		{
			...
			MY_RPTF(_CRT_WARN,"CSerialEx::ThreadProc - Unable to wait until COM event has arrived\n");
			return m_lLastError;
		}
#else
		...
		if (!m_fStopping)
		{
			...
			if (!::ClearCommError(m_hFile,&dwErrors,0))
			{
				...
				// Issue an error and quit
				MY_RPTF(_CRT_WARN, "CSerialEx::ThreadProc - Unable to obtain COM status\n");
			}
