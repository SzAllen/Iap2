...
void CHTMLStatic::OnPaint() 
{
	...
	CRect rctBefore;
	dc.GetClipBox(rctBefore);

	CRgn rgnControl;
	rgnControl.CreateRectRgn( rctClient.left, rctClient.top, rctClient.right, rctClient.bottom);
	...
	CRgn rgnNormal;
	rgnNormal.CreateRectRgn( rctBefore.left, rctBefore.top, rctBefore.right, rctBefore.bottom);
	dc.SelectClipRgn(&rgnNormal);

	if ( m_htmlDrawer.GetAnchorCount() > 0 )
